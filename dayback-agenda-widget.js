...
const NOTIFICATIONS_ENABLED = true; // Set to true to show upcoming event notifications on your device
...
const appDomain = "https://app.dayback.com";
const widgetDomain = "https://dbk-widget.tellen.me";
...
const showTomorrow =
  now.getHours() >= HOUR_THRESHOLD_FOR_NEXT_DAY ? true : false;
...
const clickUrl = USE_BOOKMARK_IN_BROWSER ? loadUrl : appDomain;
...
if (
  (now.getHours() <= 23 &&
    compareDate.getHours() >= 0 &&
    compareDate.getHours() < REFRESH_INTERVAL / 60) ||
  (HOUR_THRESHOLD_FOR_NEXT_DAY !== 0 &&
    now.getHours() < HOUR_THRESHOLD_FOR_NEXT_DAY &&
    compareDate.getHours() >= HOUR_THRESHOLD_FOR_NEXT_DAY)
) {
...
try {
  ...
  eventsPayload = JSON.parse(await req.loadString());
...
function buildWidget(eventsPayload) {
  const events = eventsPayload.sort(compare);
  let linesShown = 0;
  ...
  for (let i = 0; i < events.length; i++) {
    const title = stripHtml(events[i].title);
    ...
    if (linesShown < MAX_LINES) {
      ...
      if (
        !refreshDateUpdated &&
        !events[i].allDay &&
        sortTimes(refreshDate, endDate) > 0 &&
        sortTimes(endDate, now) > 0
      ) {
        refreshDateUpdated = true;

        // Set refresh date to end date but add a minute to make sure it is enough in the future to trigger
        let refreshEndDate = new Date(endDate);
        refreshEndDate.setMinutes(refreshEndDate.getMinutes() + 1);

        refreshDate = refreshEndDate; // Make sure we don't set refresh date to now
      }
    } else {
      if (!footerSet) {
    ...
    if (NOTIFICATIONS_ENABLED) {
      ...
      notificationDate.setMinutes(
        notificationDate.getMinutes() - NOTIFICATIONS_MINUTES_BEFORE,
        0,
        0,
      );
...
function setNoEvents() {
  const noEventsText = showTomorrow
    ? TRANSLATIONS["No events scheduled"]
    : TRANSLATIONS["No events remaining"];
...
function setWidgetHeader() {
  ...
  const calendarIcon = showTomorrow
    ? SFSymbol.named("calendar.circle")
    : SFSymbol.named("clock");
...
function stripHtml(htmlText) {
  return decodeHtmlEntity(stripHtmlTags(htmlText));
}
